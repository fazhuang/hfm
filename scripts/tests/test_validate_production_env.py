...
Validates scripts/validate-production-env.py and scripts/deploy-gate.sh:
  ...
  - the read-only migration verification accepts an isolated database at 0018
...
def _sqlite_at_revision(db_file: Path, revision: str) -> None:
    """Migrate an isolated sqlite file to the requested revision (0013/0014/0018)."""
...
def test_migration_verification_accepts_0018(tmp_path: Path) -> None:
    db_file = tmp_path / "at0018.db"
    _sqlite_at_revision(db_file, "0018")
    errors = validator.verify_migration(
        BACKEND_DIR, f"sqlite+aiosqlite:///{db_file}", "0018"
    )
    assert errors == []
...
def test_deploy_gate_test_env_0018_passes(tmp_path: Path) -> None:
