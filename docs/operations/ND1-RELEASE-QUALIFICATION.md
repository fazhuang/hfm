...
| DATABASE_ROLLBACK | Move the database to an earlier revision | NOT a normal rollback: 0016 → 0015 downgrade is not supported as a release path (migration content is additive and not safely reversible). Do not attempt migration downgrade for rollback |
